...
| A11 Language tooling | Partial | Machine-readable diagnostics, catalog, explain, formatting replacements, manifests, shared Project Index schema 1, an initial standard stdio LSP with full buffer sync/UTF-16/completion/hover/diagnostics/formatting, and an unreleased VS Code client candidate with an explicit-executable VSIX gate | Run a real extension-host gate, add another editor client, consume Project Index for navigation, add debounced theme-aware semantic checks, and prove equality between CLI/editor errors. |

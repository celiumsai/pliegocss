...
| A11 Language tooling | Partial | Machine-readable diagnostics, catalog, explain, formatting replacements, manifests, shared Project Index schema 1, and an initial standard stdio LSP with full buffer sync, UTF-16 conversion, completion, hover, syntax/format diagnostics, and safe whole-literal formatting edits | Package editor clients, consume Project Index for navigation, add debounced theme-aware semantic checks, and prove equality between CLI/editor errors. |

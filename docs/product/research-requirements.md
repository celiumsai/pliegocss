...
| A10 Tailwind analyzer | Partial | Tailwind 4.3.2 benchmark remains pinned; migration project schema 1 now binds exact Tailwind CSS, config, plugin, and template files, resolves typed relative seams, inventories literal/dynamic template candidates plus closed unsupported config/plugin hooks, and has a three-case authored contract runner without executing JavaScript | Add broader template/arbitrary-value/interpolation and source/safelist graphs, conflicts, and reviewed real-project fixtures with precision/recall evidence. |

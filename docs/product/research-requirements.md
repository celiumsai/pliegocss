...
| A10 Tailwind analyzer | Partial | Tailwind 4.3.2 benchmark corpus remains pinned; `pliego-css-source` migration inventory schema 1 now binds one exact Tailwind CSS v4 entry file, classifies Tailwind imports/Preflight, `@source` including dynamic inline generation, theme/utility/variant/reference syntax, and unsupported `@apply`/plugin/config seams without executing JavaScript | Add complete template utility/arbitrary-value/dynamic interpolation inventory, source/safelist and config/plugin graphs, conflicts, project-level CLI report, and reviewed migration fixtures. |
| A11 Language tooling | Partial | Machine-readable diagnostics, catalog, explain, formatting replacements, manifests, and shared Project Index schema 1 | LSP transport consuming the index, completion, hover, diagnostics, explain, and equality between CLI/editor errors. |
| A12 Performance corpus | Partial | Gate A/B, Rust-check, watch, pruning, media merge, fuzz, parallel determinism, package-size gates | Content-addressed 1k/10k/100k graphs, 10-package monorepo, 50/500 scoped-file fixtures, cold/warm/incremental memory and p50/p95, hosted release-commit evidence. |

...
| R0.3 Compatibility policy | Stable diagnostic codes and reproducible preserved/transformed/fallback/degraded/blocked decisions based on versioned official data. | **Partial:** exact official data and bounded typed/rule-feature decisions are implemented; complete CSS classification and browser/fallback evidence are missing. |
| R0.4 Budgets | Bytes, rules, selectors, specificity, and semantic duplication, with route/package/layer attribution and regression deltas. | **Partial:** schema 1 closes exact artifacts plus verified Asset Plan file/layer aggregation and ownership-backed package/composed-route aggregation with cross-bundle fingerprints. `audit --ownership` exact-plan binding, exclusive package groups, overlapping route views, input ledger, and budget gates pass locally in Debian WSL2. Usage analysis schemas 1/2 add explicit generated-StyleId observation/deadness, whole-StyleId pruning evidence, and hash-bound bundle-qualified retention without treating runtime absence as deletion proof. Route views remain outside Control Manifest partitions; generic CSS/declaration/token coverage, hosted evidence, and the final release replay remain open. |
| R0.5 Token graph | Aliases, cycle rejection, themes, coverage, hashes, and typed provenance. | **Partial:** the canonical graph, DTCG format/Resolver bridges, aliases, derived values, deprecation provenance, cycle rejection, validated theme permutations, graph hashes, transitive coverage, integrity-bound control artifact, direct CLI selection, bundle-plan schema-2 selection, and Cargo build-macro selection are implemented and locally verified through the clean package gate. Accessibility policy schema 1 declares contrast relationships against the graph; hosted evidence remains missing. |
| R0.6 Basic accessibility guards | Declared contrast pairs and configurable motion/focus policies with verified/unverified/manual coverage. | **Partial — implementation gate locally green:** direct CSS and verified Asset Plans evaluate the closed schema-1 policy for contrast, motion, focus visibility, forced colors, and input modality; policy and optional TokenGraph bytes are integrity-bound, findings preserve verified/unverified/manual-required states and exceptions, and control token measurements record declared pair count. Evidence: 30 control tests, nine CLI audit tests, one Asset Plan test, strict Clippy, and the dirty package gate. Corpus precision/recall, browser/manual validation, hosted runners, and native macOS/ARM64 evidence remain open, so no WCAG certification or complete release claim is made. |
...
| R0.8 Audit bridges | At least read-only inventory for Sass, Tailwind, and CSS Modules, including unsupported/dynamic constructs and Preflight reliance. | **Partial — bounded Rust producer implemented:** migration inventory schema 1 accepts one explicit Sass, Tailwind CSS v4 entry, or CSS Modules source; binds path/bytes/SHA-256 and exact construct spans; classifies static/dynamic/unsupported syntax; and records Tailwind Preflight reliance without executing original toolchains. Complete project/import/template/config/plugin/composition graphs, CLI reporting, real migration fixtures, and reversible codemods remain missing. |

...
| `distribution` | adoption | **tailwind-advantage** | Published npm packages and documented install paths are available today. | The exact-version 0.1.0-rc.3 crate graph, CLI, and LSP are prepared for crates.io and Rust 1.85 replay; publication, repository assets, and editor-client distribution remain candidate gates. | non-blocking |

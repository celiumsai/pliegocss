...
The checked evidence used Node.js 22.13.0,
Rust 1.85, Debian WSL2, and a target directory on the Linux filesystem; package and source bytes
still came from the mounted checkout.
...
- Replacing the `pc!` value with an invalid spacing token made both compilers fail. The CSS SHA-256,
  manifest SHA-256, both modification times, served class, semantic padding, page content, and reload
  generation remained at the last valid `p-4` state after a 2,000 ms quiet window.
...
- The first 20-sample causal audit exposed a delayed duplicate PliegoRS rebuild. The watcher now
  requires a stable 500 ms project snapshot and restarts that quiet window when any source or final
  publication changes. The gate fails on a transient missing/stale asset or any extra generation.

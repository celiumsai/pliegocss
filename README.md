...
- The typed theme registry, schema-1 TOML configuration, build-time `theme!` bridge, theme-scoped
  ...
  0.12.4 Windows and Debian hosts; complete semantic diagnostic parity remains open.
...
- The publishable boundary candidate is now sixteen exact-version crates in six dependency waves.
  ...
  exact clean `6f28e66` replay packaged and compiled all sixteen extracted archives and passed its
  registry-shaped Rust 1.85 consumer. `pliego-css-lsp` was 25,468 bytes,
  `pliego-css-build` 59,916 bytes, `pliego-cssc` 61,341 bytes, and `pliego-css-control` 61,078
  bytes. Nothing has been uploaded, and repository/registry ownership gates remain open.

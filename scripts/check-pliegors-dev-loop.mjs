...
// PliegoCSS remains compatible with Rust 1.85; this cross-repository harness
// uses the stricter Rust 1.86 MSRV declared by the pinned PliegoRS revision.
const TOOLCHAIN = "1.86.0";
...
function materializeFixture(runtime) {
  ...
  for (const dependency of ["pliego-dom", "pliego-ssg"]) {
    const declaration = `${dependency} = "=0.0.2"`;
    if (!manifest.includes(declaration)) {
      fail(`fixture manifest is missing exact published dependency ${declaration}`);
    }
  }
  const cargoOverridePaths = ["pliego-dom", "pliego-ssg"].map((crate) =>
    normalizedPath(join(PLIEGORS_ROOT, "crates", crate)),
  );
  const cargoOverrideDirectory = join(project, ".cargo");
  mkdirSync(cargoOverrideDirectory);
  writeFileSync(
    join(cargoOverrideDirectory, "config.toml"),
    `# Generated by the PliegoCSS cross-repository development-loop gate.\npaths = ${JSON.stringify(cargoOverridePaths)}\n`,
  );
  manifest = manifest
    .replace(
      'pliego-dom = "=0.0.2"',
      `pliego-dom = { version = "=0.4.0-beta.1", path = ${JSON.stringify(normalizedPath(join(PLIEGORS_ROOT, "crates", "pliego-dom")))} }`,
    )
    .replace(
      'pliego-ssg = "=0.0.2"',
      `pliego-ssg = { version = "=0.4.0-beta.1", path = ${JSON.stringify(normalizedPath(join(PLIEGORS_ROOT, "crates", "pliego-ssg")))} }`,
    );
  writeFileSync(manifestPath, manifest);
  run("cargo", [`+${TOOLCHAIN}`, "generate-lockfile", "--manifest-path", manifestPath], {
    cwd: project,
  });
...
function htmlState(html) {
  ...
  const reload = /<script\b[^>]*data-pliego-reload[^>]*>[\s\S]*?<\/script>/u.exec(html)?.[0];
  const generation = Number(/\bconnect\((\d+)\)\}\)\(\)<\/script>/u.exec(reload ?? "")?.[1]);
  ...
  return waitUntil(
    ...
    async () => {
      ...
      try {
        ...
        const payload = /^data: (.+)$/mu.exec(body)?.[1];
        if (!payload) return null;
        const update = JSON.parse(payload);
        const generation = typeof update === "number" ? update : update.generation;
